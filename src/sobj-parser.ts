// SOBJ Parser: Parses tokens from sobj-lexer.ts into an AST
import { Token, TokenType, tokenize } from './sobj-lexer';
import { throwParserError } from './sobj-error';

// AST Node Types
type SObjValue = string | number | SObjObject | SObjFunction | SObjRawJS;

interface SObjObject {
  type: 'Object';
  properties: SObjProperty[];
  types?: Map<string, string>; // key -> type
  functions?: Map<string, any>; // key -> function
}

interface SObjProperty {
  key: string;
  value: SObjValue;
  valueType?: string; // e.g., 'string', 'number', etc.
  inferredType?: string;
}

interface SObjFunction {
  type: 'Function';
  name: string;
  params: string[];
  body: SObjRawJS | SObjObject;
}

interface SObjRawJS {
  type: 'RawJS';
  code: string;
}

// Parser implementation
class Parser {
  tokens: Token[];
  pos: number = 0;

  constructor(tokens: Token[]) {
    this.tokens = tokens;
  }

  peek(offset = 0): Token | undefined {
    return this.tokens[this.pos + offset];
  }

  next(): Token {
    return this.tokens[this.pos++];
  }

  expect(type: TokenType): Token {
    const token = this.next();
    if (token.type !== type) {
      throw new Error(`Expected token ${type}, got ${token.type} at line ${token.line}, column ${token.column}`);
    }
    return token;
  }

  parse(): SObjObject {
    this.expect(TokenType.ObjectStart);
    const properties = this.parseProperties();
    this.expect(TokenType.ObjectEnd);
    return { type: 'Object', properties };
  }

  parseProperties(): SObjProperty[] {
    const properties: SObjProperty[] = [];
    while (this.peek() && this.peek()!.type !== TokenType.ObjectEnd && this.peek()!.type !== TokenType.CloseBracket) {
      if (this.peek()!.type === TokenType.Comma) {
        this.next(); // skip comma
        continue;
      }
      // Skip NJS blocks at the property level (for function bodies)
      if (this.peek()!.type === TokenType.NJSBlockStart) {
        this.next(); // skip NJSBlockStart
        if (this.peek() && this.peek()!.type === TokenType.RawJS) {
          this.next(); // skip RawJS
        }
        if (this.peek() && this.peek()!.type === TokenType.NJSBlockEnd) {
          this.next(); // skip NJSBlockEnd
        }
        continue;
      }
      properties.push(this.parseProperty());
    }
    return properties;
  }

  parseProperty(): SObjProperty {
    let keyToken = this.next();
    let key: string;
    let valueType: string | undefined = undefined;
    let inferredType: string | undefined = undefined;
    if (keyToken.type === TokenType.Identifier || keyToken.type === TokenType.FunctionName) {
      key = keyToken.value;
    } else {
      throwParserError(`Expected property key, got ${keyToken.type}`);
    }
    // Optional type annotation
    if (this.peek() && this.peek()!.type === TokenType.Colon) {
      this.next(); // skip :
      const typeToken = this.expect(TokenType.Identifier);
      valueType = typeToken.value;
    }
    // Assignment or function
    if (this.peek() && this.peek()!.type === TokenType.Equals) {
      this.next(); // skip =
      const value = this.parseValue();
      // Infer type if not explicitly set
      if (!valueType) {
        if (typeof value === 'string') inferredType = 'string';
        else if (typeof value === 'number') inferredType = 'number';
        else if (value && value.type === 'Object') inferredType = 'object';
        else if (value && value.type === 'RawJS') inferredType = 'any';
      }
      return { key, value, valueType, inferredType };
    } else if (this.peek() && this.peek()!.type === TokenType.OpenParenthesis) {
      // Function property
      const func = this.parseFunction(key);
      return { key, value: func, valueType: 'function' };
    } else {
      throwParserError(`Unexpected token after property key: ${this.peek()?.type}`);
    }
  }

  parseValue(): SObjValue {
    const token = this.peek();
    if (!token) throwParserError('Unexpected end of input while parsing value');
    if (token.type === TokenType.String) {
      this.next();
      return token.value;
    } else if (token.type === TokenType.Number) {
      this.next();
      return Number(token.value);
    } else if (token.type === TokenType.RawJS) {
      this.next();
      return { type: 'RawJS', code: token.value };
    } else if (token.type === TokenType.OpenBracket || token.type === TokenType.ObjectStart) {
      return this.parseObject();
    } else {
      throwParserError(`Unexpected token type for value: ${token.type}`);
    }
  }

  parseObject(): SObjObject {
    // Accept both OpenBracket and ObjectStart for nested objects
    if (this.peek()!.type === TokenType.OpenBracket) {
      this.next();
    } else if (this.peek()!.type === TokenType.ObjectStart) {
      this.next();
    }
    const properties = this.parseProperties();
    // Collect types and functions
    const types = new Map<string, string>();
    const functions = new Map<string, any>();
    for (const prop of properties) {
      // Only check .type if value is an object
      if (
        prop.valueType === 'function' &&
        typeof prop.value === 'object' &&
        prop.value !== null &&
        'type' in prop.value &&
        (prop.value as any).type === 'Function'
      ) {
        functions.set(prop.key, prop.value);
      }
      if (prop.valueType) {
        types.set(prop.key, prop.valueType);
      } else if (prop.inferredType) {
        types.set(prop.key, prop.inferredType);
      }
    }
    if (this.peek()!.type === TokenType.CloseBracket) {
      this.next();
    } else if (this.peek()!.type === TokenType.ObjectEnd) {
      this.next();
    }
    return { type: 'Object', properties, types, functions };
  }

  parseFunction(name: string): SObjFunction {
    this.expect(TokenType.OpenParenthesis);
    const params: string[] = [];
    while (this.peek() && this.peek()!.type !== TokenType.CloseParenthesis) {
      const paramToken = this.expect(TokenType.Identifier);
      params.push(paramToken.value);
      if (this.peek() && this.peek()!.type === TokenType.Comma) {
        this.next();
      }
    }
    this.expect(TokenType.CloseParenthesis);
    this.expect(TokenType.ArrowFunction);
    // Function body can be a block or NJS block
    let body: SObjRawJS | SObjObject;
    if (this.peek()!.type === TokenType.NJSBlockStart) {
      this.next(); // skip NJSBlockStart
      const rawjsToken = this.expect(TokenType.RawJS);
      this.expect(TokenType.NJSBlockEnd);
      body = { type: 'RawJS', code: rawjsToken.value };
    } else if (this.peek()!.type === TokenType.OpenBracket || this.peek()!.type === TokenType.ObjectStart) {
      // Try to parse a block, but if it only contains a single NJS block, treat as RawJS
      const startPos = this.pos;
      this.next(); // consume the bracket
      if (this.peek() && this.peek()!.type === TokenType.NJSBlockStart) {
        this.next(); // skip NJSBlockStart
        const rawjsToken = this.expect(TokenType.RawJS);
        this.expect(TokenType.NJSBlockEnd);
        if (this.peek() && (this.peek()!.type === TokenType.CloseBracket || this.peek()!.type === TokenType.ObjectEnd)) {
          this.next(); // consume closing bracket
          body = { type: 'RawJS', code: rawjsToken.value };
          return { type: 'Function', name, params, body };
        } else {
          // Not a single NJS block, fallback to object parsing
          this.pos = startPos; // rewind
        }
      } else {
        this.pos = startPos; // rewind
      }
      body = this.parseObject();
    } else {
      throwParserError('Expected function body after =>');
    }
    return { type: 'Function', name, params, body };
  }
}

// Exported parse function
export function parseSObj(tokens: Token[]): SObjObject {
  const parser = new Parser(tokens);
  return parser.parse();
}

// Example usage (for testing):
/*const ast = parseSObj(tokenize(`{
  key1: string = "hello",
  key2 = 42,
  $onchange (key, value) => {
    [*njs]
    if (key == "key1") {
      console.log("key1 changed from ", value.old, " to ", value.new);
    }
    [njs*]
  }
}`));
console.dir(ast, { depth: null });
*/
